...
public:
	...
	void Draw(const Vector2& position, std::string& image, Color color, int _width = 0, int _height = 0, bool _rev = false);
	...
	void SetPrintCollision(bool _print) { print_collision = _print; }
	bool GetPrintCollision() { return print_collision; }
	...
	void SetConsoleFontSize(HANDLE& handle, int size);
